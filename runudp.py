...
def run_scripts():
    server_process = subprocess.Popen(["python3", "UDPServer.py"], preexec_fn=os.setsid)
    time.sleep(1)
    
    client_process = subprocess.Popen(["python3", "UDPClient.py"], preexec_fn=os.setsid)
...
def plot_times():
    ...
    plt.title('UDP Round Trip Times per Packet')
    plt.show()
